...
export type RangeMode = "pct" | "price";
export type ZoneStatus = "unset" | "below" | "in_zone" | "above";

export interface HoldingRanges {
  tp_mode?: RangeMode | null;
  tp_low?: number | null;
  tp_high?: number | null;
  sl_mode?: RangeMode | null;
  sl_low?: number | null;
  sl_high?: number | null;
}
...
export interface Holding {
  ...
  tp_mode?: RangeMode | null; tp_low?: number | null; tp_high?: number | null;
  sl_mode?: RangeMode | null; sl_low?: number | null; sl_high?: number | null;
  tp_status?: ZoneStatus; sl_status?: ZoneStatus;
  /** 五日偏向分：自低点涨幅 − 自高点回撤；正=偏涨，负=偏跌 */
  chg5?: number | null;
  high5?: number | null;
  low5?: number | null;
...
export const api = {
  ...
  addHolding: (code: string, shares: number, cost: number, ranges?: HoldingRanges) =>
    request<PortfolioData>("/portfolio/holding", "POST", { code, shares, cost, ...ranges }),
  updateHoldingRanges: (code: string, ranges: HoldingRanges) =>
