...
class HoldingRangesIn(BaseModel):
    """止盈/止损区间；全部留空表示清空该持仓的两侧区间。"""
    tp_mode: str | None = None
    tp_low: float | None = None
    tp_high: float | None = None
    sl_mode: str | None = None
...
def _ranges_dict(obj: HoldingIn | HoldingRangesIn) -> dict:
    return {
        "tp_mode": obj.tp_mode, "tp_low": obj.tp_low, "tp_high": obj.tp_high,
        "sl_mode": obj.sl_mode, "sl_low": obj.sl_low, "sl_high": obj.sl_high,
    }


@app.get("/api/portfolio")
def portfolio_get():
    """持仓 + 实时盈亏 + 止盈止损状态 + 近5日动能。"""
    try:
...
def portfolio_add(h: HoldingIn):
    ...
    try:
        return {"data": pf.add_holding(code, h.shares, h.cost, ranges=_ranges_dict(h))}
    except ValueError as e:
        raise HTTPException(400, str(e)) from e


@app.patch("/api/portfolio/holding/ranges")
def portfolio_update_ranges(body: HoldingRangesIn, code: str = Query(...)):
    """更新某持仓的止盈/止损区间（不改正本）。body 全空则清空区间。"""
    code = (code or "").strip()
    if not code.isdigit() or len(code) != 6:
        raise HTTPException(400, "代码必须是 6 位数字")
    try:
        return {"data": pf.update_holding_ranges(code, _ranges_dict(body))}
    except ValueError as e:
        raise HTTPException(400, str(e)) from e
...
def global_indices():
    """全球指数快照（美股 / 港股 / 韩股 KOSPI 等）—— A 股看隔夜外围脸色。缓存 5 分钟。"""
    try:
